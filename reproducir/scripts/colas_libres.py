...
integrando en J (un extremo o punto donde se comporta como x^a da t^-(a+1)):
  ...
  * en J = 0, C ~ sqrt(J) (amplitud epicíclica), así que dPhi va como
    F_eq(0) s(0) J^{1/2}: t^{-3/2} si F_eq(0) > 0 con s = 1, t^{-2} con
    s = sqrt(J/J_max), y t^{-7/2} si F_eq ~ J^2;
...
en t = n tau_1 con tau_1 = 2 pi/(ancho de la banda), y la pendiente logarítmica
entre los dos últimos tiempos. La convergencia se comprueba con media malla en J.

Uso:  python3 colas_libres.py [--nu 2400] [--nq 128] [--ntau 1 2 5 10]
"""
import os, sys, argparse, time, warnings, numpy as np
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from equilibrio import Equilibrio, invertir, F_perfil
...
def main():
    ...
    ap.add_argument('--ntau', type=float, nargs='+', default=[1, 2, 5, 10],
                    help='tiempos en unidades de tau_1; la pendiente usa los dos últimos')
    arg = ap.parse_args()
    NTAU = arg.ntau
    validar()
    ...
    casos = [
        ('gauss sigma=0.05',        dict(forma='gauss', sigma=0.05), 'plana', 0.05),
        ('J^2 (J_t-J)^3, J_t=0.138', dict(forma='politropo', jt=0.138, k=3.0, m=2.0), 'plana', 0.138),
        ('(J_t-J)^3, J_t=0.138',    dict(forma='politropo', jt=0.138, k=3.0, m=0.0), 'plana', 0.138),
        ('(J_t-J)^3, J_t=0.138',    dict(forma='politropo', jt=0.138, k=3.0, m=0.0), 'suave', 0.138),
    ]
    B = prueba(0.05, 0.05)                           # j1 = sj1 = 0.36 J_t, como 11_landau
    print(f'isócrono L0 = {L0:g}; B(J) con j1 = sj1 = 0.05; rms de dPhi en r in [3, 15]\n')
    cab = (f'{"F_eq":>26} {"pert":>6} {"tau_1":>6} | ' +
           ' '.join(f'{"h1 %gtau" % n:>9}' for n in NTAU) + f' {"pend.":>6} | ' +
           ' '.join(f'{"dPhi %gtau" % n:>9}' for n in NTAU) + f' {"pend.":>6} {"conv.":>6}')
    print(cab); print('-'*len(cab))
    for nombre, kw, pert, jsop in casos:
        F = F_perfil(J, **kw)
        s = np.ones_like(J) if pert == 'plana' else np.sqrt(J/jsop)
        if kw['forma'] == 'gauss':                   # banda: F > 1 % del máximo
            sop = F >= 0.01*F.max()
            ancho = Om[sop].max() - Om[sop].min()
        else:
            ancho = 1.0/C_ISO**3 - 1.0/(jsop + C_ISO)**3
        tau1 = 2*np.pi/ancho
        t = np.array([0.0] + [n*tau1 for n in NTAU])
        fase = np.exp(-1j*np.outer(t, Om))
        res = []
        for paso in (1, 2):                          # malla completa y media malla
            uu = u[::paso]; w = trapecio(uu)*2*J_end*uu
            g = (F*s)[::paso]*w
            h = np.abs(fase[:, ::paso] @ (B(J[::paso])*g))
            Z = (C[:, ::paso]*g[None, :]) @ fase[:, ::paso].T      # (radio, tiempo)
            ph = np.sqrt(np.mean(np.abs(Z)**2, axis=0))
            res.append((h/h[0], ph/ph[0]))
        (h, ph), (_, ph2) = res
        conv = np.max(np.abs(ph2[1:]/ph[1:] - 1))
        ph_ = lambda x: np.log(x[-1]/x[-2])/np.log(NTAU[-1]/NTAU[-2])
        print(f'{nombre:>26} {pert:>6} {tau1:6.0f} | ' +
              ' '.join(f'{x:9.1e}' for x in h[1:]) + f' {ph_(h):6.1f} | ' +
              ' '.join(f'{x:9.1e}' for x in ph[1:]) + f' {ph_(ph):6.1f} {conv:6.0e}')
    print(f'\npend. = d log(amplitud)/d log(t) entre {NTAU[-2]:g} y {NTAU[-1]:g} tau_1; conv. = cambio relativo '
          'máximo de dPhi con media malla en J.')
